	<?php
	$types = $_POST['types'];
	$yearworked = $_POST['yearworked'];

	$niveau	= $_POST['niveau'];

	$printName = "WORKED_LIST-IN_".$niveau."_".$types;
	...
	while ($affiche = $voir->fetch()) {
		...
		$searchStd = $dtb->query('SELECT * FROM tbl_2024_etudiant WHERE annee_scolaire = "'.$yearworked.'" AND etude_envisage ="'.$types.'" AND annee_etude ="'.$niveau.'" ORDER BY student_id');

		while($stdA = $searchStd->fetch()) {

	?>
<div style="page-break-inside: avoid;" class="pt-1">
