...
$result = $dtb->query('SELECT *,
		   COUNT(DISTINCT CASE WHEN ins.status = "Interne" AND std.sex = 1 THEN ins.student_id END) AS Interne_H,
		   COUNT(DISTINCT CASE WHEN ins.status = "Interne" AND std.sex = 0 THEN ins.student_id END) AS Interne_F,
		   COUNT(DISTINCT CASE WHEN ins.status = "Bungalow" AND std.sex = 1 THEN ins.student_id END) AS Bungalow_H,
		   COUNT(DISTINCT CASE WHEN ins.status = "Bungalow" AND std.sex = 0 THEN ins.student_id END) AS Bungalow_F,
		   COUNT(DISTINCT CASE WHEN (ins.status = "Externe" OR ins.status = "") AND std.sex = 1 THEN ins.student_id END) AS Externe_H,
		   COUNT(DISTINCT CASE WHEN (ins.status = "Externe" OR ins.status = "") AND std.sex = 0 THEN ins.student_id END) AS Externe_F
