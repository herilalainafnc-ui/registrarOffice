...
if ($crs['cours_category'] == 0){
	...
	echo "-";
				...
				?></td>
					...
					<td class="p-0 text-center bg-green-700 w-[60px]"><a href="<?=$app_base?>/app/.student/recup-cours?student_id=<?=$student_id?>&id=<?=$id?>&as=<?=$a.$s?>&idSupprCours=<?=$note_id?>&user_id=<?=$rg_id?>">Restaurer</a></td>
					<td class="p-0 text-center bg-red-700 w-[60px]"><a href="<?=$app_base?>/app/.student/del-cours.definitive?student_id=<?=$student_id?>&id=<?=$id?>&as=<?=$a.$s?>&idSupprCours=<?=$note_id?>&user_id=<?=$rg_id?>&id_cours=<?=$id_cours?>">Effacer</a></td>
