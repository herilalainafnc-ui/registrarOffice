		...
		<table class="simpleTbl mb-1">
			...
			<thead class="<?=$bg_one_color?> <?=$txt_one_color?>">
				<tr>
					...
					<th class="w-20">Cr�dits</th>
					<th class="w-20">Cat�gorie</th>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				$('.form-no-refrech<?=$nbr.$a.$s?>').on('submit',function(e){
					...
					// Arrondir à 2 décimales avant envoi
					var inp = $('.g<?=$nbr.$a.$s?>');
					var nv = parseFloat(inp.val().replace(',', '.'));
					if (!isNaN(nv) && nv != -2) {
						inp.val(Math.round(nv * 100) / 100);
					}
...
if (($crs['cours_category'] == 0) OR ($crs['cours_category'] == "G�n�ral")) {
	$gradeGen = $crs['grade'];
	$nbrGen++;
}else{
	$gradeGen = 0;
...
if (($crs['cours_category'] == 1) OR ($crs['cours_category'] == "Majeur") OR ($crs['cours_category'] == 0) OR ($crs['cours_category'] == "G�n�ral")) {
	 ...
	 ?>			
			...
			<tfoot class="<?=$bg_one_color?> <?=$txt_one_color?>">
				<tr>
					...
					<th class="px-2"></th>
					<th class="px-2"><?php if(($nbr-1)<1){echo 0;}else{ if ($rg_level < 7) { echo round($tnote,2);}else{ echo "<em>masqu�</em>";}}?></th>
					<th><?php if(($nbr-1)<1){echo 0;}else{ if ($rg_level < 7) { echo round($tnotecredit,2);}else{ echo "<em>masqu�</em>";}}?></th>
...
<?php 
	...
	if ($rg_level < 7) { echo "<a class='px-2'>".$grade_work_educ."</a>"; }else{ echo "<em>masqu�</em>";}
 ...
 ?>
				...
				<tr class="<?=$bg_four_color?> text-right">
					<td colspan="4">Remarque acad�mique</td>
					<td class="<?=$bg_six_color?> <?=$txt_three_color?> px-0 text-left">
...
<?php 
	...
	if ($rg_level < 7) { echo "<a class='px-2'>".$grade_remark_acad."</a>"; }else{ echo "<em>masqu�</em>";}
 ...
 ?>
				...
				<tr class="<?=$bg_four_color?> text-right">
					<td colspan="4">Note de participation � l'exercice de chapelle et � la semaine de pri�re</td>
					<td class="<?=$bg_six_color?> <?=$txt_three_color?> px-0 text-left">
...
<?php 
	...
	if ($rg_level < 7) { echo "<a class='px-2'>".$grade_chapel_part."</a>"; }else{ echo "<em>masqu�</em>";}
...
<?php 
 ...
 ?>
				<!-- <tr>
					<th colspan="4" class="text-right">Moyenne G�n�rale(faux)</th>
				...
				<tr>
					...
					<!-- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! -->
					<th class="px-2"><?php if ($rg_level < 7) { if($nbrMaj != 0){echo round(($moyenMajSem = ($tTMaj/$tcreditMaj)),2);}else{echo 0;$moyenMajSem =0;}}else{ echo "<em>masqu�</em>";}?></th>
					<!-- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! -->
				</tr>
				
				<!--  -->
				<tr>
					<th colspan="4" class="text-right">Moyenne G�n�rale</th>
					<th class="px-2 bg-cyan-700 text-white"><?php if ($rg_level < 7) { if($nbr != 0){echo round(($moyenGenSem = $tnotecredit/$tcredit),2);}else{echo 0;$moyenGenSem =0;}}else{ echo "<em>masqu�</em>";}?></th>
				</tr>
	...
	<table class="simpleTbl mb-1 w-full">
		<tbody class=" <?=$bg_two_color?> <?=$txt_one_color?>">
			<tr>
				...
				<td class="px-2 w-2/12 text-bold"><?php if ($rg_level < 7) {round(($cumulWorkNote*20)/((($a-1)*2)*20),2);}else{ echo "<em>masqu�</em>";}?></td>
			</tr>
			<!-- <tr>
				<td class="p-1 w-8/12 text-right">Nemarque acad�mique cumulative</td>
				<td class="px-2 w-2/12 text-bold"><?php if ($rg_level < 7) {round(($cumulremarkAcad*20)/((($a-1)*2)*20),2);}else{ echo "<em>masqu�</em>";}?></td>
			</tr> -->
			<tr>
				<td class="p-1 w-8/12 text-right">Note de participation � l'exercice de chapelle et � la semaine de pri�re cumulative</td>
				<td class="px-2 w-2/12 text-bold"><?php if ($rg_level < 7) {round(($cumulChapel*20)/((($a-1)*2)*20),2);}else{ echo "<em>masqu�</em>";}?></td>
			</tr>
		</tbody>
	</table>
	<table class="simpleTbl mb-1 w-full">
		<thead class="bg-slate-900">
			<!-- <tr>
				<th class="p-1 w-8/12 text-right">Moyenne G�n�rale Cumulative</th>
			...
			<tr>
				...
				<th class="py-1 px-2 w-2/12"><?php if ($rg_level < 7) {round(($cumulMaj*20)/((($nbrA-1)*2)*20),2);}else{ echo "<em>masqu�</em>";}?></th>
			</tr>
			
			<!--  -->
			<tr>
				<th class="p-1 w-8/12 text-right bg-cyan-700">Moyenne G�n�rale Cumulative</th>
				<th class="py-1 px-2 w-2/12 bg-cyan-700"><?php if ($rg_level < 7) {round(($cumulGen*20)/((($nbrA-1)*2)*20),2);}else{ echo "<em>masqu�</em>";}?></th>
			</tr>
		</thead>
	</table>
</div>
