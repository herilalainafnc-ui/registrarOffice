			...
			<table class="simpleTbl mb-1">
				<thead class="bg-slate-800 text-white">
					<tr>
						...
						<td class="w-20">Ann act</td>
						<td class="w-20">Ann cours</td>
						<td class="w-20">Sem cours</td>
...
if($apotr){
	...
	echo "<em style='color:red'>Etudiant non inscrit dans la base!!</em>";
					...
					?></td>
						<td class="c<?=$nbr.$i;?>"><?php
if(!empty($apotr['annee_etude'])){
	 if($apotr['annee_etude'] == 1){
	 	echo $apotr['annee_etude']." ère";
	 }else{
	 	echo $apotr['annee_etude']." eme";
	 }
}else{
	echo "<em style='color:red'>none</em>";
					...
					?></td>
					<td class="c<?=$nbr.$i;?>"><?php 
if(!empty($cours_table['yearlevel'])){
	 if($cours_table['yearlevel'] == 1){
	 	echo $cours_table['yearlevel']." ère";
	 }else{
	 	echo $cours_table['yearlevel']." eme";
	 }
}else{
	echo "<em style='color:red'>none</em>";
}
					...
					<td class="c<?=$nbr.$i;?>"><?=$cours_table['semester']?></td>
						...
						<td class='stp<?=$nbr.$i;?>' style="
<?php 
	if ($cours_table['grade'] == 0 OR $cours_table['grade'] == "") {
		echo "";
	}elseif ($cours_table['grade'] < 10) {
		echo "background : red; color : white; font-wheight : bold; text-align: center";
	}elseif ($cours_table['grade'] >= 10){
		echo "background : #16dd2b; color : white; font-wheight : bold; text-align: center";
	}
 ?>
						">
						<?php 
